...
function personal() {
  ...
  $('#personal-form').onsubmit = async event => {
    ...
    const payload = {
      authorization: { mode: 'private', verified: true, method: 'self_test_max_access' },
      identifiers: { email: $('#email').value.trim(), phone: $('#phone').value.trim(), username, nickname: username },
      platformCatalog: await catalog(),
