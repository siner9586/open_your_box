...
const DEFAULT_PLATFORM_CATALOG = [
  { id: 'github', name: 'GitHub', domain: 'github.com', identifiers: ['username'], recoveryEntry: 'https://github.com/password_reset', dataExportEntry: 'https://github.com/settings/export', deletionEntry: 'https://github.com/settings/admin', cleanupNote: 'Review security settings, SSH keys, tokens and public repositories.' },
...
function usernameNicknameMatrix(payload = {}, scanId = 'scan') {
  ...
  const username = String(ids.username || ids.nickname || '').trim();
  if (!username) return { findings: [], tasks: [] };
  const catalog = Array.isArray(payload.platformCatalog) ? payload.platformCatalog : [];
  const candidates = catalog.filter(p => {
    const identifiers = (p.identifiers || []).map(x => String(x).toLowerCase());
    const category = String(p.category || '').toLowerCase();
    return identifiers.some(x => ['username', 'nickname', 'github_username', 'wechat_id', 'qq_number', 'gamertag'].includes(x)) || /social|community|creator|developer|forum|gaming|video|music|livestream|work|professional|microblog|content|knowledge|lifestyle|travel|local_life/i.test(category);
  }).slice(0, 150);
...
function mergeExtraFindings(report, extra = [], subjectType = 'personal') {
  ...
  for (const item of extra) { const key = `${item.source}:${item.category}:${item.title}`; if (!seen.has(key)) { report.findings.push(item); seen.add(key); } }
...
function mergeTasks(report, tasks = []) {
  if (!tasks.length) return report;
  const seen = new Set((report.accountTasks || []).map(t => `${t.platformId}:${t.taskType}:${t.platformName}`));
  for (const task of tasks) { const key = `${task.platformId}:${task.taskType}:${task.platformName}`; if (!seen.has(key)) { report.accountTasks.push(task); seen.add(key); } }
  return report;
}
...
export async function listReports(context = {}) {
  ...
  if (d?.prepare) { try { const rows = await d.prepare('select id, job_id, report_type, risk_score, created_at from reports order by created_at desc limit 50').all(); return response({ reports: (rows.results || []).map(r => ({ id: r.id, scanId: r.job_id, reportType: r.report_type, riskScore: r.risk_score, createdAt: r.created_at })) }); } catch (error) { return jsonError('D1_READ_FAILED', 500, 'Could not read reports from D1.', safeMsg(error)); } }
